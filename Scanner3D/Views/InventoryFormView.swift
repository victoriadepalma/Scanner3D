...
struct InventoryFormView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Add USDZ", isPresented: $vm.showUSDZSource, actions: {
            Button("Select file"){
                vm.selectUSDZSource = .fileImporter
            }
            Button("Object Capture"){
                vm.selectUSDZSource = .objectCapture
            }
        })
        .fileImporter(isPresented: .init(get: {
            vm.selectUSDZSource == .fileImporter
        }, set: { _ in
            vm.selectUSDZSource = nil
    ...
    var arSection: some View{
        Section("AR Model"){
            ...
            if let progress = vm.uploadProgress,
               case let .uploading(type) =
                vm.loadingState,
               progress.totalUnitCount > 0 {
                VStack {
                    ProgressView(value:
                                    progress.franctionCompleted) {
                        Text("Uploading \(type == .usdz ? "USDZ": "Thumbnail")file \(Int(progress.franctionCompleted * 100))%")
                    }
        ...
        //para que el usuario no le de upload otra vez
        .disabled(vm.loadingState != .none)
    }
    func viewAR(url: URL) {
