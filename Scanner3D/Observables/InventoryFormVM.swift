...
//  Scanner3D
//
//  Created by Victoria De Palma on 4/24/24.
//
...
class InventoryFormViewModel: ObservableObject {
    ...
    @Published var selectUSDZSource: USDZSourceType?
    ...
    func uploadUSDZ(fileURL: URL) async {
        ...
        uploadProgress = .init(franctionCompleted: 0, totalUnitCount: 0, completedUnitCount: 0)
        loadingState = .uploading(.usdz)
        
        defer {loadingState = .none}
        do{
            //Upload a FirebaseStorage
            let storageRef = Storage.storage().reference()
            let usdzRef = storageRef.child("\(id).usdz")
            
            try await usdzRef.putDataAsync(data,metadata: .init(dictionary: ["contentType" : "model/vnd.usd+zip"])) {[weak self] progress in
                guard let self, let progress else {return}
                self.uploadProgress = .init(franctionCompleted: progress.fractionCompleted, totalUnitCount: progress.totalUnitCount, completedUnitCount: progress.completedUnitCount)
            }
...
struct UploadProgress {
    var franctionCompleted: Double
    var totalUnitCount: Int64
    var completedUnitCount: Int64
}

//enum UploadCoordinator: Equatable {
//    case usdz, thumbnail
//}
